...
class VJPlayer {
    ...
    init() {
        // Controller <-> browser communication must not depend on the external
        // YouTube IFrame API becoming ready.  Start polling immediately so
        // commands/feedback continue to work even while YouTube is loading or
    ...
    onPlayerReady(playerId, event) {
        ...
        // Keep keyboard/mouse focus away from YouTube's iframe.  This does not
        // modify the cross-origin YouTube UI itself, but helps prevent its
        // center play/pause overlay from being kept visible by iframe focus.
        try {
            ...
            // UI suppression must never interfere with playback.
        }
    ...
    onPlayerStateChange(playerId, event) {
        ...
        // Keep the original, reliable loop behavior.  After restarting, blur
        // the iframe element so YouTube's center overlay is less likely to stay
        // visible.  Playback never depends on the blur operation.
        if (state === YT.PlayerState.ENDED && playerId === this.currentPlayer) {
            this.players[playerId].playVideo();
            try {
                const iframe = this.players[playerId] && typeof this.players[playerId].getIframe === 'function'
                    ? this.players[playerId].getIframe()
                    : null;
                if (iframe) {
                    iframe.blur();
                }
                ...
                // Never let overlay suppression break looping.
            }
        }
    }
